...
def load_dashboard_data(base_dir: Path, selected_category: str = "全部") -> dict:
    ...
    print("当前csv列名：", segment_df.columns.tolist())
    ...
    # 打印全部列名，查看真实字段
    print("segment_df 所有列名：", segment_df.columns.tolist())

    max_churn_row = segment_df.loc[segment_df["流失率"].idxmax()]

    # 手动写固定列名，后续根据控制台打印结果修改引号内内容
    # 打印列名调试
    print("segment_df 所有列名：", segment_df.columns.tolist())
    ...
    # 真实列名 TenureGroup（生命周期分组）
    col_name = "TenureGroup"
    max_stage = max_churn_row[col_name]
    max_churn_rate = max_churn_row["流失率"]

    insight = f"用户生命周期阶段中，{max_stage}阶段用户流失率最高，流失率达到{max_churn_rate:.2f}%，建议针对该阶段用户推送留存运营活动。"
